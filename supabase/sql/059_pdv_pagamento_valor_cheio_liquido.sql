-- Permite valor pago (cheio) > total da venda quando a entrada líquida fecha o total.
-- Casos:
-- 1) Soma(valor) = total → ok (fluxo normal; líquido pode ser menor por taxa MDR).
-- 2) Soma(valor) >= total e Soma(valor_liquido) = total → ok (juros/taxa na maquininha).
-- Financeiro continua usando a soma dos valores líquidos.
-- Execute após 058.
...
begin
  ...
  if p_pagamentos is not null
    ...
    if not public.fin_pagamentos_cobrem_total(v_soma_pagamentos, v_total_liquido, v_total) then
      raise exception
        'Pagamentos não fecham o total da venda (%). Informe valor pago = total, ou valor pago maior com entrada líquida = total.',
        v_total;
    end if;
