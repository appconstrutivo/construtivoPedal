-- Remove datas sentinela (ex.: 0001-01-01) que disparam atraso falso na verificação de 30 dias.
-- O manual só considera data_compra a partir de 1990.

update public.bicicletas
set data_compra = null,
    updated_at = timezone('utc', now())
where data_compra is not null
  and extract(year from data_compra) < 1990;
...
begin
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'prazo_em', case
        when r.tipo = 'verificacao_30' and r.sequencia = 1 and v_data_compra is not null
          then to_char(v_data_compra + 30, 'YYYY-MM-DD')
        else null
      end,
  ...
  return jsonb_build_object(
    ...
    'prazo_verificacao_30', case
      when v_data_compra is not null then to_char(v_data_compra + 30, 'YYYY-MM-DD')
      else null
    end
  );
end;
