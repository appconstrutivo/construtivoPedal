-- Restaura fin_data_operacional_br em financeiro_registrar_recebimento (regrediu na 053).
-- Data pura (date::timestamptz) grava meia-noite UTC → dia anterior no Brasil → fora do filtro "Hoje".

-- Backfill vendas/movimentações de OS afetadas
...
where cr.venda_id = v.id
  ...
  and v.realizada_em = cr.data_recebimento::timestamptz;
