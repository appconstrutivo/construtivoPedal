-- Recebimento de contas a receber com pagamento misto (como PDV).

alter table public.financeiro_contas_receber
  drop constraint if exists financeiro_contas_receber_forma_check;

alter table public.financeiro_contas_receber
  add constraint financeiro_contas_receber_forma_check
  check (
    forma_pagamento is null
    or forma_pagamento in ('dinheiro', 'pix', 'credito', 'debito', 'outro', 'misto')
  );

drop function if exists public.financeiro_registrar_recebimento(uuid, uuid, text, date);
...
begin
  ...
  if p_pagamentos is not null
    ...
    v_qtd_pagamentos := 1;
    v_ultima_forma := v_forma;
    v_soma_pagamentos := v_cr.valor;
  end if;
